...
# function to find the action and replace with passed param
def read_sim_file(param):
	regex_str = " "+args.parameter+" = "
	temp_var = -1
	for ele in data_list:
		# search for line which has the action to be changed
		if ele[0] == args.actions:
			for var in ele:
...
if __name__ == '__main__':
	...
	parser.add_argument('-w', '--workflow', action="store", required=False, help=" Workflow selected")
	parser.add_argument('-s', '--simulation_file', action="store", required=True, help="Simulation to edit")
	parser.add_argument('-a', '--actions', action="store", required=True, help="Action to edit")
	parser.add_argument('-d', '--parameter', action="store", required=True, help="Action's parameter to change")
	parser.add_argument('-v', '--variable', action="append", required=True, help="Value to put for the parameter selected, can send in more than one.")
