...
class Simulation:
	...
	def updateGraphAfterExecution( self, executed_action ):
		if executed_action in list(self.workflow):
			...
			# 
			for neighbor, datadict in self.workflow.adj[ executed_action ].items():
				print(neighbor)
		else:
	...
	def run( self ):
		...
		while len(self.next_action) > 0:
			...
			if self.verifyActionOutput( current_action ):
				write_log(self.log_pathfilename, "The outputs of Action" + current_action  + " have been sucessfully verified.")
