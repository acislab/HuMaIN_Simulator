...
# Description: 	Gets the simulated results of the crowdsourced transcription of Scientific 
# 				Name. Three different users processed every image.
##########################################################################################
...
if __name__ == '__main__':
	""" Gets the simulated results of the crowdsourced transcription of Scientific Name
	"""
	parser = argparse.ArgumentParser("Gets the simulated results of the crowdsourced transcription of Scientific Name.")
	parser.add_argument('-s', '--specimens', action="store", required=True, help="Text file with the list of specimens for which no scientific name has been extracted.")
	...
	# -s /home/ialzuru/Summer2019/HuMaIN_Simulator/selfie/results/scientific_name/sn_dict_extr/rejected/rejected.tsv 
	# -f /home/ialzuru/Summer2019/HuMaIN_Simulator/datasets/aocr_mix100/crowd/terms/zooniverse/scientific_name.tsv
	# -m duration 
	# -o /home/ialzuru/Summer2019/HuMaIN_Simulator/selfie/results/scientific_name/sn_crowd/sn_crowd.tsv
	#
	# python3 sn_crowd.py -s /home/ialzuru/Summer2019/HuMaIN_Simulator/selfie/results/scientific_name/sn_dict_extr/rejected/rejected.tsv -f /home/ialzuru/Summer2019/HuMaIN_Simulator/datasets/aocr_mix100/crowd/terms/zooniverse/scientific_name.tsv -m duration -o /home/ialzuru/Summer2019/HuMaIN_Simulator/selfie/results/scientific_name/sn_crowd/sn_crowd.tsv
	################################################################################################################################
	...
	verify_create_file( args.output_file, 'The output file, for the transcribed scientific names, could not be created (' + args.output_file + ').', parser, 6 )
	# Metrics
	...
	for index, row in df_crowd.iterrows():
		...
		if filename_txt in specimens_list:
			crowd_data_string += filename_txt + "\t" + row['value1'] + "\t" + row['value2'] + "\t" + row['value3'] + "\n"
	...
	for m_name in args.metric:
		...
		for index, row in df_in_me.iterrows():
			filename_txt = row['filename'].replace(".jpg", ".txt")
			if filename_txt in specimens_list:
				metric_string += filename_txt + "," + str(row['sec1']) + "," + str(row['sec2']) + "," + str(row['sec3']) + "\n"
