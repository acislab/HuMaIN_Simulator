...
if __name__ == '__main__':
	...
	"""
	parser = argparse.ArgumentParser("Get the simulated results of the crowdsourced transcription of Recorded By.")
	parser.add_argument('-s', '--specimens', action="store", required=True, help="Text file with the list of specimens for which no recorded by data has been extracted.")
	...
	# print("Are the specimens being read correctly?")
	# print("These are the specimens!!!")
	# print(specimens_list)
	# print(len(specimens_list))  #100   --- all ok here ---


    ################################################################################################################################
	# Load in a dataframe the crowdsourced data
	df_crowd = pd.read_csv( args.crowd_data, sep='\t', names=["filename", "recorded_by1", "recorded_by2"])
	...
	for index, row in df_crowd.iterrows():
		...
		if row['filename'] in specimens_list:
			crowd_data_string += row['filename'] + "\t" + row['recorded_by1'] +  "\t" + row['recorded_by2'] + "\n"
